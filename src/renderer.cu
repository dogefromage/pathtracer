...
struct context_t {
    ...
    PLATFORM context_t(const scene_t *_scene, const bvh_t *_bvh, const lst_t *_lst, config_t _cfg)
        : scene(_scene), bvh(_bvh), lst(_lst), cfg(_cfg) {
    }
};

static PLATFORM void get_camera_ray(Ray &ray, const __restrict__ scene_t *scene, float u, float v) {
...
static PLATFORM void initialize_safe_ray(Ray &ray, const Vec3 &origin, const Vec3 &dir, const Vec3 &normal) {
...
static PLATFORM void intersect(const __restrict__ bvh_t *bvh, const __restrict__ scene_t *scene, const Ray &ray,
                               intersection_t &hit) {
...
static PLATFORM Vec3 sample_triangle_uniform(rand_state_t &rstate, const Vec3 &a, const Vec3 &b, const Vec3 &c) {
    float u1, u2;
    do {
...
static PLATFORM void sample_area_light(area_light_sample_t &out, context_t &c, const Vec3 &shadow_pos,
...
static PLATFORM void sample_light_source(light_source_sample_t &out, context_t &c, const Ray &ray, const intersection_t &hit) {
...
static PLATFORM float evaluate_direct_p(context_t &c, const Ray &ray, const intersection_t &hit) {
