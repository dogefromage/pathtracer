...
static void update_node_bounds(const Scene &scene, BVH &bvh, uint32_t nodeIndex) {
...
static void print_stats(const Scene &scene, BVH &bvh, uint32_t nodeIndex, bvh_stats_t &stats) {
    log_trace("\nbvh_t stats:\n");
...
void BVH::build(const Scene &scene) {
    ...
    // primitiveCount = scene.faces.count;
    // maxNodeCount = 2 * primitiveCount - 1;

    nodeCount = 0;
    nodes.items = (bvh_node_t *)malloc(sizeof(bvh_node_t) * maxNodeCount);
    assert(nodes.items && "malloc");
    nodes.count = maxNodeCount;

    // mutable primitive list for sorting faces
    indices.items = (uint32_t *)malloc(sizeof(uint32_t) * primitiveCount);
    assert(indices.items && "malloc");
    indices.count = primitiveCount;

    for (uint32_t i = 0; i < primitiveCount; i++) {
        indices[i] = i;
    }
    // calculate centroids, accesses work with scene face indices
    centroids.items = (Vec3 *)malloc(sizeof(Vec3) * primitiveCount);
    assert(centroids.items && "malloc");
    centroids.count = primitiveCount;

    for (uint32_t i = 0; i < primitiveCount; i++) {
        centroids[i] = calculate_face_centroid(scene, scene.faces[i]);
    }

    uint32_t rootIndex = nodeCount++;
    bvh_node_t &root = nodes[rootIndex];
    root.start = 0;
    root.end = primitiveCount;
    update_node_bounds(scene, *this, rootIndex);
    subdivide(scene, *this, rootIndex, stats, 1);

    calculate_stats(scene, *this, rootIndex, stats);
    print_stats(scene, *this, rootIndex, stats);

    // not needed anymore
    free(centroids.items);
...
void BVH::device_from_host(const BVH &h_bvh) {
    log_info("Copying bvh_t to device... \n");

    *this = h_bvh;

    size_t totalSize = 0;
    totalSize += copy_device_fixed_array(&this->nodes, &h_bvh.nodes);
    totalSize += copy_device_fixed_array(&this->indices, &h_bvh.indices);
