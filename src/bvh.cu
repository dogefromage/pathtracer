...
static void
update_node_bounds(const obj_scene_data* scene, BVH* bvh, uint32_t nodeIndex) {
...
static void
subdivide(const obj_scene_data* scene, BVH* bvh, uint32_t nodeIndex) {
    BVHNode* node = &bvh->nodes[nodeIndex];
...
__host__ void
bvh_build(BVH* bvh, const obj_scene_data* scene) {
    printf("Building BVH... ");
    ...
    bvh->nodeCount = 0;
    bvh->nodes = (BVHNode*)malloc(sizeof(BVHNode) * bvh->maxNodeCount);
...
__host__ int
bvh_copy_device(BVH** d_bvh, const BVH* h_bvh) {
    ...
    curr_bytes = sizeof(BVHNode) * m_bvh.maxNodeCount;
    ...
    curr_bytes = sizeof(BVH);
    total_bytes += curr_bytes;
    err = cudaMalloc(d_bvh, curr_bytes);
    if (check_cuda_err(err)) return err;
...
__host__ int
bvh_free_device(BVH* d_bvh) {
    cudaError_t err;
    BVH m_bvh;
    err = cudaMemcpy(&m_bvh, d_bvh, sizeof(BVH), cudaMemcpyDeviceToHost);
    if (check_cuda_err(err)) return err;
    err = cudaFree(d_bvh);
    if (check_cuda_err(err)) return err;
...
intersect_aabb(const AABB* aabb, const Ray* ray, mfloat_t min_t) {
